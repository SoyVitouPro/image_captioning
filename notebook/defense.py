...
for epoch in range(num_epochs): 
    ...
    if bleu4 > best_bleu4:
        best_bleu4 = bleu4
        epochs_no_improve = 0
        torch.save(encoder.state_dict(), 'encoder_final_defensepart2.pth')
        torch.save(decoder.state_dict(), 'decoder_final_defensepart2.pth')
    else:
        epochs_no_improve += 1
...
plt.tight_layout()
plt.savefig("training_evaluation_metrics_defensepart2.png", dpi=300, bbox_inches='tight')
plt.show()

print(f"\n📊 Training completed after {actual_epochs} epochs")
print(f"📈 Best BLEU-4 score: {best_bleu4:.4f}")
print(f"💾 Models saved as 'encoder_final_defensepart2.pth' and 'decoder_final_defensepart2.pth'")
