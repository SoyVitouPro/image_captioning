...
for epoch in range(num_epochs):
    ...
    if bleu4 > best_bleu4:
        best_bleu4 = bleu4
        epochs_no_improve = 0
        torch.save(encoder.state_dict(), 'encoder_final_v3.pth')
        torch.save(decoder.state_dict(), 'decoder_final_v3.pth')
    else:
        epochs_no_improve += 1
        print(f"🔁 No improvement in BLEU-4 for {epochs_no_improve} epoch(s).")
...
plt.tight_layout()
plt.savefig("training_evaluation_metrics_finalv3.png", dpi=300)
plt.show()
