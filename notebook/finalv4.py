...
completed_epochs = 0  # Track actual completed epochs
...
for epoch in range(num_epochs):
    ...
    if bleu4 > best_bleu4:
        best_bleu4 = bleu4
        epochs_no_improve = 0
        torch.save(encoder.state_dict(), 'encoder_final_v4.pth')
        torch.save(decoder.state_dict(), 'decoder_final_v4.pth')
    else:
        epochs_no_improve += 1
...
# Use actual completed epochs for plotting
epochs = list(range(1, completed_epochs + 1))
print(f"\n📊 Training completed after {completed_epochs} epochs (out of {num_epochs} planned)")
print(f"📊 Plotting metrics for {len(epochs)} completed epochs")

# Ensure all metric lists have the same length as completed epochs
def truncate_metrics(metric_list, target_length):
    return metric_list[:target_length] if len(metric_list) >= target_length else metric_list

# Truncate all metrics to match completed epochs
train_losses = truncate_metrics(train_losses, completed_epochs)
wer_scores = truncate_metrics(wer_scores, completed_epochs)
cer_scores = truncate_metrics(cer_scores, completed_epochs)
bleu1_scores = truncate_metrics(bleu1_scores, completed_epochs)
bleu2_scores = truncate_metrics(bleu2_scores, completed_epochs)
bleu3_scores = truncate_metrics(bleu3_scores, completed_epochs)
bleu4_scores = truncate_metrics(bleu4_scores, completed_epochs)

# Verify lengths match
print(f"📊 Metrics lengths verification:")
print(f"   Epochs: {len(epochs)}")
print(f"   Train Losses: {len(train_losses)}")
print(f"   WER Scores: {len(wer_scores)}")
print(f"   BLEU Scores: {len(bleu4_scores)}")

# Only plot if we have data
if completed_epochs > 0 and len(train_losses) > 0:
    plt.figure(figsize=(16, 10))

    # WER and CER
    plt.subplot(2, 2, 1)
    if len(wer_scores) > 0:
        plt.plot(epochs[:len(wer_scores)], wer_scores, label='WER', marker='o')
    if len(cer_scores) > 0:
        plt.plot(epochs[:len(cer_scores)], cer_scores, label='CER', marker='x')
    plt.xlabel('Epoch')
    plt.ylabel('Score')
    ...
    if len(bleu1_scores) > 0:
        plt.plot(epochs[:len(bleu1_scores)], bleu1_scores, label='BLEU-1', marker='o')
    if len(bleu2_scores) > 0:
        plt.plot(epochs[:len(bleu2_scores)], bleu2_scores, label='BLEU-2', marker='x')
    if len(bleu3_scores) > 0:
        plt.plot(epochs[:len(bleu3_scores)], bleu3_scores, label='BLEU-3', marker='s')
    if len(bleu4_scores) > 0:
        plt.plot(epochs[:len(bleu4_scores)], bleu4_scores, label='BLEU-4', marker='^')
    plt.xlabel('Epoch')
    ...
    if len(train_losses) > 0:
        plt.plot(epochs[:len(train_losses)], train_losses, label='Train Loss', color='orange', marker='o')
    plt.xlabel('Epoch')
    plt.ylabel('Loss')
    ...
    plt.text(0.1, 0.7, f"Completed Epochs: {completed_epochs}/{num_epochs}", fontsize=12)
    plt.text(0.1, 0.6, f"Best BLEU-4: {best_bleu4:.4f}", fontsize=12)
    plt.text(0.1, 0.5, f"Final Train Loss: {train_losses[-1]:.4f}" if train_losses else "N/A", fontsize=12)
    plt.text(0.1, 0.4, f"Final WER: {wer_scores[-1]:.4f}" if wer_scores else "N/A", fontsize=12)
    plt.text(0.1, 0.3, f"Final CER: {cer_scores[-1]:.4f}" if cer_scores else "N/A", fontsize=12)
    if epochs_no_improve >= early_stopping_patience:
        plt.text(0.1, 0.2, "⏹️ Early Stopping Triggered", fontsize=12, color='red')
    else:
        plt.text(0.1, 0.2, "✅ Training Completed", fontsize=12, color='green')
    plt.xlim(0, 1)
    plt.ylim(0, 1)
    plt.axis('off')

    plt.tight_layout()
    plt.savefig(f"training_evaluation_metrics_finalv4_epochs{completed_epochs}.png", dpi=300)
    plt.show()
    print(f"✅ Plot saved as: training_evaluation_metrics_finalv4_epochs{completed_epochs}.png")
else:
    print("⚠️ No data to plot - training may have stopped before first epoch completed")
